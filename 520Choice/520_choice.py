...
def button(text, x, y, w, h, color, screen):
    ...
    font = pygame.font.Font(os.path.split(os.path.realpath(__file__))[0] + r"\font\ziti.ttf", 20)
...
def title(text, screen, scale, color=(0, 0, 0)):
    font = pygame.font.Font(os.path.split(os.path.realpath(__file__))[0] + r"\font\ziti.ttf", WIDTH // (len(text) * 2))
...
def show_like_interface(text, screen, color=(255, 0, 0)):
    ...
    font = pygame.font.Font(os.path.split(os.path.realpath(__file__))[0] + r"\font\simkai.ttf", WIDTH // (len(text)))
    textRender = font.render(text, True, color)
    textRect = textRender.get_rect()
...
def main():
    ...
    pygame.display.set_caption('FROM一个爱你很久的小哥哥')
    ...
    while running:
        ...
        # 加载图片
        img = pygame.image.load(os.path.split(os.path.realpath(__file__))[0] + r'\imgs\3.jpg')
        ...
        pygame.display.update()
        clock.tick(60)
    show_like_interface('我就知道小姐姐你也喜欢我，我们快去领证吧~', screen, color=(0, 0, 0))
